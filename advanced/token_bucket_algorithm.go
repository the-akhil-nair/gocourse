...
func NewRateLimiter(rateLimit int, refillTime time.Duration) *RateLimiter {
	rl := &RateLimiter{
		// Using empty struct{} to save memory as it occupies zero bytes and used only for signaling.
		// When we want to check if token available and we don't care about the value itself.
...
func (rl *RateLimiter) startRefill() {
	...
	for {
		...
		case <-ticker.C:
			select {
			// It will only add token if there is space in the channel (i.e., not full).
...
func main() {
	...
	for range 10 {
		if rateLimiter.allow() {
			fmt.Printf("Request allowed %v tokens left %v\n", len(rateLimiter.tokens), time.Now())
		} else {
