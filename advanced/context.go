...
import (
	"context" // context is a type that carries deadlines, cancellation signals, and other request-scoped values across API boundaries and between processes.
	"fmt"
	"log"
	"time"
)
...
func main() {
	// context are used to manage lifecycle of processess and to signal when operations should be canceled or timed out.
...
// Root context where other contexts derive from
// Will be mentioned from top level of application
// 	ctx = context.Background()
...
// 	time.Sleep(3 * time.Second)
// After 1 second the context will be canceled due to timeout, but only show the message once below is called.
...
// Used when you are not sure which context to use or when the context will be added later.
// Returns a TODO context and is empty ctx.
// 	todoContext := context.TODO()
//
// // Returns a Background context and is empty ctx.
