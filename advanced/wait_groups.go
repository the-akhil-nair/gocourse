...
func worker(id int, tasks <-chan int, results chan<- int, wg *sync.WaitGroup) {
	...
	for task := range tasks {
		fmt.Printf("WorkerID %d processing task %d\n", id, task)
...
// 	defer wg.Done() // Correct practice to signal that the worker is done and decrement the WaitGroup counter
...
// 	// wg.Add(numWorkers - 1) // Whenever a wait group is defered the wg counter will be decreased by 1. This line results in negative WaitGroup counter.
// 	wg.Add(numWorkers) // THIS IS THE CORRECT WAY OF ADDING COUNTER TO WAIT GROUPS
// 	// wg.Add(numWorkers + 1) // This line will cause the program to wait for one extra worker that doesn't exist and create a deadlock.
